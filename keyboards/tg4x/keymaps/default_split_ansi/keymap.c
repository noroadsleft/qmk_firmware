...
#if defined(KEYBOARD_tg4x_rev1)
    #pragma GCC error "This keymap uses the LAYOUT_ansi macro, which is only compatible with revs. 2 and 2.1."
#endif
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    // |-------'-,-----'-,-----'-,-----'-,-----'-,-----'-,-----'-,-----'-,-----'-,-----'-,-----'-,-----'-------|
         KC_TRNS ,KC_HOME,KC_PGUP,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_LBRC,KC_RBRC,KC_EQL ,KC_BSLS,   KC_ENT    ,
    // |---------'---,---'---,---'---,---'---,---'---,---'---,---'---,---'---,---'---,---'---,---'---,---------|
...
void update_led(void) {
    if (host_keyboard_leds() & (1<<USB_LED_CAPS_LOCK)) { //if caps lock is on
        ...
        switch (biton32(layer_state)) {
            case _BL: //when the base layer is active, turns the LEDs white
                rgblight_sethsv_range(0,0,150,1,7);
                break;
            case _FL: //when the function layer is active, turns the LEDs green
                rgblight_sethsv_range(85,200,150,1,7);
                break;
            case _OL:
                rgblight_sethsv_range(170,200,150,1,7);
                break;
        ...
        switch (biton32(layer_state)) { //same code as above
            case _BL:
                rgblight_sethsv_range(0,9,150,1,7);
                break;
            case _FL:
                rgblight_sethsv_range(85,200,150,1,7);
                break;
